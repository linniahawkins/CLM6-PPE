    ...
    def write(self,paramdir,nldir):
        ...
        for param in self.get_names():
            if self._paramdict[param].location == "P":
                precision_str = getattr(self._paramdict[param], "_precision", None)
                if precision_str and precision_str.startswith('.') and precision_str.endswith('f'):
                    decimals = int(precision_str[1:-1])  # e.g., '.2f' → 2
                    val = np.round(self._paramdict[param].value, decimals)
                else:
                    val = self._paramdict[param].value
